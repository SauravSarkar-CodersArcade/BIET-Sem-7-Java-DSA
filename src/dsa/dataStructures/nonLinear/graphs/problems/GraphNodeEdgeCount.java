package dsa.dataStructures.nonLinear.graphs.problems;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;
public class GraphNodeEdgeCount {
    public static int countNodes(List<List<Integer>> adjList){
        return adjList.size();
    }
    public static int countEdges(List<List<Integer>> adjList){
        int edges = 0;
        for (List<Integer> neighbour : adjList){
            edges += neighbour.size();
        }
        // Since this is an undirected graph, each edge is counted twice
        // So, we divide the edge count by 2
        return edges / 2;
    }
    public static void main(String[] args) {
        List<List<Integer>> adjList = new ArrayList<>();
        // We will create the graph manually
        adjList.add(Arrays.asList(1,3)); // Node 0
        adjList.add(Arrays.asList(0,2,3)); // Node 1
        adjList.add(Arrays.asList(1,3,4)); // Node 2
        adjList.add(Arrays.asList(0,1,2,4)); // Node 3
        adjList.add(Arrays.asList(2,3)); // Node 4
        int nodes = countNodes(adjList);
        int edges = countEdges(adjList);
        System.out.println("Number of nodes: " + nodes);
        System.out.println("Number of edges: " + edges);
    }
}
